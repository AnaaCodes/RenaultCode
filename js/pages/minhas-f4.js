...
document.querySelector('#clearFilters')?.addEventListener('click',()=>{searchInput.value='';dateFilter.value='Todos';sectorFilter.value='Todos';if(teamUser||managerial)select('all');else{statusFilter.value='Todos';render();}});
document.querySelector('#newF4Button')?.addEventListener('click',()=>location.href='./nova-f4.html');
const params=new URLSearchParams(location.search); const flowParam=params.get('flow'); if(params.get('returned')==='me'&&(teamUser||managerial))select('returned'); else if(flowParam&&(teamUser||managerial)&&['validation','commercial','technical','manager','cve','approved','rejected'].includes(flowParam))select(flowParam); else { if(teamUser||managerial)select('all'); else render(); }
