...
function sync(){
  document.querySelectorAll('[data-count-filter]').forEach(el=>el.textContent=data.filter(i=>matches(i,el.dataset.countFilter)).length);
  document.querySelectorAll('[data-stat]').forEach(el=>{const status=el.dataset.stat;el.textContent=status==='Todos'?data.length:data.filter(i=>i.status===status).length;});
}
...
function selectSupplierStatus(status){statusFilter.value=status;statsGrid.querySelectorAll('[data-status]').forEach(c=>{const selected=c.dataset.status===status;c.classList.toggle('is-selected',selected);c.setAttribute('aria-pressed',selected)});render();}
configure();sync();statsGrid.querySelectorAll('[data-filter]').forEach(c=>c.onclick=()=>select(c.dataset.filter));
if(!(teamUser||managerial))statsGrid.querySelectorAll('[data-status]').forEach(c=>c.onclick=()=>selectSupplierStatus(c.dataset.status));
[searchInput,dateFilter,sectorFilter].forEach(el=>el.addEventListener(el.tagName==='INPUT'?'input':'change',render));
statusFilter.addEventListener('change',()=>teamUser||managerial?select(statusFilter.value):selectSupplierStatus(statusFilter.value));
document.querySelector('#clearFilters')?.addEventListener('click',()=>{searchInput.value='';dateFilter.value='Todos';sectorFilter.value='Todos';if(teamUser||managerial)select('all');else{dashboardFlowFilter=null;selectSupplierStatus('Todos');}});
